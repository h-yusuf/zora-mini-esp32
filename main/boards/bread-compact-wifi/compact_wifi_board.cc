...
private:
    ...
    void InitializeButtons() {
        ...
        // Triple-click toggles the lamp GPIO, so the LED can be checked at any
        // time without a reboot and without the cloud being reachable.
        boot_button_.OnMultipleClick([this]() {
            if (lamp_ == nullptr) {
                return;
            }
            lamp_->Toggle();
            ESP_LOGI(TAG, "Lamp toggled to %s (GPIO%d)", lamp_->power() ? "on" : "off",
                     (int)lamp_->gpio_num());
        }, 3);
    ...
    // Blink the lamp GPIO a few times right after boot. This is the simplest
    // possible end-to-end check of the breadboard: it exercises one GPIO plus
    // the ground rail, with no bus or clock involved.
    void LampSelfTest() {
        if (lamp_ == nullptr || lamp_->gpio_num() == GPIO_NUM_NC) {
            ESP_LOGW(TAG, "Lamp self test skipped: no lamp GPIO configured");
            return;
        }
        ESP_LOGI(TAG, "Lamp self test: blinking GPIO%d 5x", (int)lamp_->gpio_num());
        for (int i = 0; i < 5; i++) {
            lamp_->Set(true);
        ...
        ESP_LOGI(TAG, "Lamp self test done");
    }
    ...
    void InfoTickerLoop() {
        // Quick hardware check right after boot: cycle the LED colors so the
        // wiring can be eyeballed, then sample the mic directly.
        GetLed()->SelfTest();
        LampSelfTest();
        MicSelfTest();
