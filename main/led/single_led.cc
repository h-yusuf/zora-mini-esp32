...
void SingleLed::SelfTest() {
    if (led_strip_ == nullptr) {
        ESP_LOGW(TAG, "Self test skipped: LED not initialized (GPIO_NUM_NC?)");
        return;
    }
    struct { const char* name; uint8_t r, g, b; } steps[] = {
        {"red", HIGH_BRIGHTNESS, 0, 0},
        {"green", 0, HIGH_BRIGHTNESS, 0},
        {"blue", 0, 0, HIGH_BRIGHTNESS},
        {"white", HIGH_BRIGHTNESS, HIGH_BRIGHTNESS, HIGH_BRIGHTNESS},
    };
    for (auto& step : steps) {
        ESP_LOGI(TAG, "Self test: %s", step.name);
        SetColor(step.r, step.g, step.b);
        TurnOn();
