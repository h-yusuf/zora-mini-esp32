...
bool AudioService::ReadAudioData(std::vector<int16_t>& data, int sample_rate, int samples) {
    ...
    // TEMP DEBUG: print raw mic signal level roughly once per second.
    static int mic_level_debug_counter = 0;
    if (++mic_level_debug_counter >= 300) {
        mic_level_debug_counter = 0;
        int32_t peak = 0;
        int64_t sum_abs = 0;
        for (int16_t s : data) {
            int32_t v = s < 0 ? -s : s;
        ...
        int32_t avg = data.empty() ? 0 : static_cast<int32_t>(sum_abs / data.size());
        ESP_LOGI(TAG, "Mic level: peak=%d avg=%d samples=%d", (int)peak, (int)avg,
                 (int)data.size());
    }
