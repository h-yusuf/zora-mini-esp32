...
int NoAudioCodec::Read(int16_t* dest, int samples) {
    ...
    // TEMP DEBUG: dump a few raw 32-bit I2S words once per second so we can
    // tell real audio from a floating / misaligned data line.
    static int raw_dump_counter = 0;
    if (++raw_dump_counter >= 1000 && samples >= 4) {
        raw_dump_counter = 0;
        ESP_LOGI(TAG, "raw i2s: %08lx %08lx %08lx %08lx",
                 (unsigned long)bit32_buffer[0], (unsigned long)bit32_buffer[1],
                 (unsigned long)bit32_buffer[2], (unsigned long)bit32_buffer[3]);
    }

    for (int i = 0; i < samples; i++) {
        // INMP441-class mics send 24-bit data left-aligned in a 32-bit slot,
        // so full scale occupies the top 24 bits. Shifting by 16 maps that to
        // int16 without clipping; smaller shifts saturate on normal speech.
